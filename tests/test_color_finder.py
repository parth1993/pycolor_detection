...
def test_color_dictionary_find_closest_color(color_dict):
    ...
    closest_color = color_dictionary.find_closest_color(
        (254, 1, 1)
    )  # Almost Red
    assert closest_color == "Red"


def test_color_finder_process_image(
    mocker, sample_image_path, color_dict, simple_strategy
):
    # Mocking ImageProcessor to return a solid color image
    mocker.patch(
        "background_removal.ImageProcessor.read_image",
        return_value=np.zeros((100, 100, 3), dtype=np.uint8),
    )
    mocker.patch(
        "background_removal.ImageProcessor.convert_to_rgb",
