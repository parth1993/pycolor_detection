...
def test_simple_removal():
    ...
    assert np.all(result[np.where((result == [0, 0, 0]).all(axis=2))] == [0, 0, 0])

def test_u2net_removal():
    # Mocking the U2NET model's output for a simple foreground-background separation
    mock_model = MagicMock()
    mock_model.return_value = MagicMock()
    mock_model.return_value.cpu.return_value.detach.return_value.numpy.return_value = np.ones((320, 320), dtype=np.float32)
